...
## Format Data for Display & Grid Analysis - New DYAD TASK
```{r format, include=F}
...
dyad_data <- dyad_pilot %>%
  mutate(targetFix1Array = ifelse(condition =="adult", dyad_pilot$data1Array[match(dyad_pilot$parent_id, dyad_pilot$unique_id)],data1Array),
         targetFix2Array = ifelse(condition =="adult", dyad_pilot$data2Array[match(dyad_pilot$parent_id, dyad_pilot$unique_id)],data2Array),
...
```{r}
fixed_data <- calculate_data2 %>%
  filter(condition == "adult", trialDisplay != 0, trialDisplay !=0.5) %>%
  group_by(sub_id, trialDisplay) %>%
  mutate(accuracy=1-mean(accuracy)) %>%
  group_by(trialDisplay) %>%
  tidyboot_mean(accuracy)

ggplot(fixed_data, aes(x= trialDisplay, y=empirical_stat, ymin=ci_lower, ymax=ci_upper))+geom_pointrange()+ggtitle("Average number of changes made when fixing grids")
```
