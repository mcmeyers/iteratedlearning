...
emds <- spread_data %>%
  filter(generation > 0) %>%
  split(paste0(.$unique_id, "_", .$trialDisplay)) %>%
  map(emd_helper) %>%
  bind_rows() 
...
ggplot(trial_data %>% filter(trialCount == 1 | trialCount ==0) %>% group_by(sub_id) %>%summarise(accuracy = mean(accuracy)), aes(x=accuracy)) + geom_histogram() +scale_x_continuous()
...
trial_data %>% 
  ...
  summarise(accuracy = mean(accuracy)) %>%
  ggplot(aes(x = generation, y = accuracy, color=as.factor(seed))) + 
  geom_point() + 
